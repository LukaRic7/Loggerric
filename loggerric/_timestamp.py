...
class Timestamp:
    ...
    **Manage global timestamps.**
    ...
    @classmethod
    def get(cls,
            dont_override_diff:bool=False,
            return_with_ansi:bool=False) -> str:                               
        """
        **Get the current timestamp.**

        *Parameters*:
        - `dont_override_diff` (bool): Determines whether this call updates
        the timestamp difference reference.
        - `return_with_ansi` (bool): USED INTERNALLY, adds suffix space,
        brackets, coloring and returns timestamp even if disabled.
        ...
        *Example*:
        ...
        if not cls._enabled and return_with_ansi:                              
            return ''
        ...
        lookup = {
            ...
            '{MI}': f'{now.minute:02d}',
            '{SS}': f'{now.second:02d}',
            '{MS}': str(now.microsecond)[0:min(3, len(str(now.microsecond)))],
            '{DH}': f'{int(dh):02d}',
            '{DM}': f'{int(dm):02d}',
            '{DS}': f'{int(ds):02d}',
            '{DN}': f'{int(dn):03d}',
    ...
    def set_format(cls, format:str) -> None:
        ...
        *Parameters*:
        ...
        Difference = Since last timestamp (unless `dont_override_diff` is
        `True` in the `Timestamp.get()` function.)
